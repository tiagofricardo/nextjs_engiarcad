...
export const getProjectImagePaths = (folder) => {
  const getImages = (folder) => {
    try {
      return fs.readdirSync(path.join("public/images/projects/", folder));
    } catch (error) {
      return "404";
    }
  };

  const filesPath = getImages(folder);

  if (filesPath !== "404") {
    const imagesData = filesPath.map((image) => {
      const dimensions = sizeOf(
        path.join(`public/images/projects/${folder}/${image}`)
      );
      return {
        url: `/images/projects/${folder}/${image}`,
